 ...
 */
import { getLastFlightDepartureDate, getNormalizedFlightSegments, normalizeFlightRequest } from './flightSegments.js';
...
function extractFlightContext(flightData: any): any {
  ...
  return {
    ...
    children: normalizedFlight?.children || 0,   // Niños 2-12 años (CHD)
    infants: normalizedFlight?.infants || 0,     // Infantes 0-2 años (INF)
...
export function enrichHotelFromFlightContext(
  ...
  if (!enriched.checkoutDate && (flightContext.returnDate || getLastFlightDepartureDate(flightContext))) {
    enriched.checkoutDate = flightContext.returnDate || getLastFlightDepartureDate(flightContext);
