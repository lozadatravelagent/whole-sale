...
-- This file combines the new migrations to apply them to the database

-- Migration 1: Add loss_reason column to leads table
ALTER TABLE leads
...
-- Migration 2: Create activities table for tracking lead and system activities
...
CREATE INDEX IF NOT EXISTS idx_activities_agency_created ON activities(agency_id, created_at DESC) WHERE deleted_at IS NULL;

-- RLS Policies
ALTER TABLE activities ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "OWNER can view all activities" ON activities;
DROP POLICY IF EXISTS "SUPERADMIN can view tenant activities" ON activities;
DROP POLICY IF EXISTS "ADMIN can view agency activities" ON activities;
DROP POLICY IF EXISTS "SELLER can view own lead activities" ON activities;
