-- ============================================================================
-- RATE LIMITING MIGRATION - EJECUTAR EN SUPABASE SQL EDITOR
-- URL: https://supabase.com/dashboard/project/ujigyazketblwlzcomve/editor
-- ============================================================================

-- LÍMITES GENEROSOS (NO AFECTAN RENDIMIENTO):
-- ✅ 100 búsquedas/hora por usuario
-- ✅ 500 búsquedas/día por usuario
-- ✅ 200 mensajes/hora por usuario
-- ✅ 1000 mensajes/día por usuario
...
CREATE TABLE IF NOT EXISTS rate_limit_config (
  ...
  -- Search limits (generosos)
  max_searches_per_hour integer DEFAULT 100,
  max_searches_per_day integer DEFAULT 500,

  -- Message/chat limits (generosos)
  max_messages_per_hour integer DEFAULT 200,
  max_messages_per_day integer DEFAULT 1000,

  -- API call limits
  max_api_calls_per_hour integer DEFAULT 150,
...
BEGIN
  ...
  CASE
    ...
    ELSE
      v_limit := 100; -- Default fallback generoso
  END CASE;
...
-- Function to cleanup old usage records
...
DROP POLICY IF EXISTS "Service role has full access to rate config" ON rate_limit_config;
DROP POLICY IF EXISTS "Users can view their own usage" ON rate_limit_usage;
DROP POLICY IF EXISTS "Service role has full access to rate usage" ON rate_limit_usage;
...
-- ============================================================================
SELECT
  '✅ Rate limiting instalado correctamente!' as status,
  COUNT(*) as tenants_configurados
FROM rate_limit_config;
