...
│                                                                         │
│   if (VITE_USE_CONTEXT_ENGINEERING):                                    │
...
│                                                                         │
│   if (USE_FUNCTION_TOOLS env || x-use-tool-loop header):                │
│     runToolLoop({                                                       │
...
│                                                                         │
│   else: legacy single-shot completion (no tools)                        │
└─────────────────────────────────────────────────────────────────────────┘
         │
         ▼
...
| **System prompt** | `C:\Users\Fran\Desktop\Projects\WholeSale\wholesale-connect-ai\supabase\functions\ai-message-parser\prompt.ts` | `PROMPT_VERSION = 'emilia-parser-v4'`. Carries `<persistence>` + `<tool_selection>` blocks; injects `MEMORY STATE` section when `memoryStateBlock` is provided. |
| **Parser entry** | `C:\Users\Fran\Desktop\Projects\WholeSale\wholesale-connect-ai\supabase\functions\ai-message-parser\index.ts` | Owns the feature-flag branch (`useToolLoop`), runs `runToolLoop`, batch-persists `save_memory_note` results, emits `[CTX-TOOL]` / `[CTX-MEMORY]`. Falls back to legacy single-shot on any loop failure. |
...
6. **Edge**: if `USE_FUNCTION_TOOLS=true` (env) **or** the request carries `x-use-tool-loop: true`, `runToolLoop` runs with the four retrieval tools + `save_memory_note`. Otherwise legacy single-shot.
...
| Flag | Where | Default | Effect when ON | Effect when OFF |
|---|---|---|---|---|
| `VITE_USE_CONTEXT_ENGINEERING` | client (Vite env, read in `useMessageHandler.ts` and `ChatFeature.tsx`) | OFF | `bootstrapStateIfMissing` runs, mode/refs are applied, `memoryStateBlock` is rendered and forwarded to the parser. | No state load, no `memoryStateBlock` sent — legacy push-context path. |
| `USE_FUNCTION_TOOLS` (env) **or** `x-use-tool-loop: true` (request header) | edge function `ai-message-parser/index.ts:310-311` | OFF (env unset, header absent) | `runToolLoop` is used with retrieval tools + `save_memory_note`. Telemetry events emit. | Single-shot Chat Completions call, no `tools` array. No `[CTX-TOOL]` events. |

Both flags default OFF. With both OFF, the parser path is byte-identical to pre–Context-Engineering behavior. Enable in staging first; roll out gradually.
