...
import { fetchPlaceDetails } from '../_shared/places/service.ts';
import type { PlaceDetailsRequest, PlacesResponseMeta } from '../_shared/places/types.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const result = await fetchPlaceDetails(body, logger);
    ...
    logger.error('places.details.error', 'Failed to fetch place details', { message });
