...
export function inferTripTypeFromSegments(
  ...
  if (segments.length === 2 && isReverseRoute(segments[0], segments[1])) {
    return 'round_trip';
  }
  return 'multi_city';
}
...
export function getLastFlightDepartureDate(flights?: FlightRequestLike | null): string | undefined {
  const segments = getNormalizedFlightSegments(flights);
  return segments.length > 0 ? segments[segments.length - 1].departureDate || undefined : undefined;
}
